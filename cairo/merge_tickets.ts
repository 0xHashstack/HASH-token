...
const owner = new Account(provider, owner_account_address, owner_private_key);
...
const claims_contract =
  "0x24e10fcdbc78671d9c34866166e737f0a786ce1fd67838086e61c954a427a44";
...
async function createBatchAirdropTransaction() {
  try {
    ...
    console.log(
      `Processing in ${totalBatches} batches of ${BATCH_SIZE} entries each`
    );
    ...
    for (let i = 0; i < addresses.length; i += BATCH_SIZE) {
      ...
      batchAddresses.forEach((addr, index) => {
        console.log(
          `${index + 1}. ${num.toHexString(addr)} - Amount: ${
      ...
      try {
        const tx = await contractInstance.transfer_tickets(
          batchAddresses,
          ticket_type
        );

        console.log(
          `Batch ${Math.floor(i / BATCH_SIZE) + 1} transaction hash:`,
          tx.transaction_hash
        );
